...
if __name__ == "__main__":              
    st.write('# “见微”系统 项目主页')
    st.write('##### **本主页中包含系统介绍（视频演示、功能原理介绍），web、安卓衍生系统链接，web登录和注册功能**')
    st.header(" “见微” 衍生应用系列场景：\n")
    ...
    st.header("系统的主页、登录、注册功能：\n")
    option = st.selectbox('功能选择',('项目介绍','注册','登录认证'))
    if(option=="项目介绍"):
         #st.header(requests.url)
         ...
         st.header("一、项目介绍")
         st.subheader("\n")
         st.write("#### 我组希望用一种新的图像语义补全模型，从数据库存储层面解决一般场景下的人脸隐私保护的问题。同时实现一种可靠的双重加密认证系统。")
         st.subheader("\n")
         st.header("二、项目流程图")
         st.subheader("\n")
         ...
         st.header("三、项目优势")
         st.subheader("\n")
         st.write("#### 1.整个认证过程中，系统中不可能也没有能力还原100%的人脸信息，这保证了整个系统中无法被盗用清晰的人脸图片。\n")
         st.write("#### 2.即使原理上能复原部分人脸信息，也无法有人独立做到这一点，需要双重密钥认证，即人脸和密钥缺一不可。\n")
         st.write("#### 3.复原后的图像绝无可能用于其它系统（其它任何的人脸系统），因为不同MAE系统的密钥转mask位置的加密算法不同且不开源，保证了同一个密钥和图像块不可能用于其它系统中。\n")
         st.write("#### 4.加密过程对用户透明，用户知道服务器存储的图像块，加密过程简单。\n")
         st.write("#### 5.人脸+密钥双重认证使得准确率大大提高基本不可能识别失误。\n")
         st.write("#### 6.服务器端不接手图像原图，只接手25%的碎片化去图像化信息\n")
         st.write("#### 7.在我们设置的一些场景下可以遮挡识别和注册，戴口罩满足疫情防控要求。\n")
         st.write("#### 8.活体防伪技术和模糊检测技术，去除不合格的脸（在本系统中为了方便演示没有）\n")
    elif(option=="注册"):
        result=None;
        st.header('注册，请上传您的人脸图片和密码')
        st.write("照片可以使用**图片上传**和**在线拍照**两种方式，使用**在线拍照**需要配置浏览器权限：")
        st.write("1.打开谷歌浏览器后地址栏输入下面链接:chrome://flags/#unsafely-treat-insecure-origin-as-secure")
        st.write("2.在打开的页面第一个输入框中输入你网页的地址，比如本页：http://mae.zkabout.xyz:8501/")
        st.write("3.点击右边按钮，选择Endble")
        st.write("4.点击右下角的重启按钮重启浏览器")
